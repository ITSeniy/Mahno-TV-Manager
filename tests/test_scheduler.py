...
def test_generate_day_handles_library_with_no_ads_or_bumpers(tmp_path):
    ...
    item_types = {row["item_type"] for row in rows}
    assert item_types <= {"episode", "card", "sms_chat", "off_air"}  # sms_chat = the nightly scene block
...
def test_night_sms_chat_block_is_one_item_for_the_whole_hour(tmp_path):
    from director.timeutil import utc_to_msk

    conn = db.connect(tmp_path / "lib.db")
    build_rich_library(conn)
    conn.execute("UPDATE series SET rotation_mode = 'random'")  # keep the day full so blocks start on time
    conn.commit()

    generate_day(conn, date(2026, 7, 6))  # Monday

    rows = conn.execute("SELECT start_time, end_time FROM program_log WHERE item_type = 'sms_chat'").fetchall()
    assert len(rows) == 1  # a single scene item spans the block, not many
    st = utc_to_msk(datetime.fromisoformat(rows[0]["start_time"]))
    et = utc_to_msk(datetime.fromisoformat(rows[0]["end_time"]))
    assert (st.hour, st.minute) == (4, 0)
    assert (et.hour, et.minute) == (5, 0)
