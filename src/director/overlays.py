...
startup to keep the *content* (logo file, ticker URL) in sync, but once
someone has hand-tuned the layout in OBS, restarting the director must not
silently reset it - only content identity is resynced on repeat calls.
"""
...
def _logo_transform(video) -> dict:
...
def _ticker_height(video) -> int:
    return round(video.base_height * TICKER_HEIGHT_FRACTION)


def _ticker_transform(video, height: int) -> dict:
    return {
...
def ensure_logo(client: obsws.ReqClient, scene_name: str, logo_path: str) -> None:
    is_new = LOGO_SOURCE not in _input_names(client)
    if is_new:
    ...
    client.set_scene_item_transform(scene_name, item_id, _logo_transform(video))


def ensure_ticker_source(client: obsws.ReqClient, scene_name: str, url: str) -> None:
    is_new = TICKER_SOURCE not in _input_names(client)
    video = client.get_video_settings()

    if is_new:
        height = _ticker_height(video)
        client.create_input(
            scene_name, TICKER_SOURCE, "browser_source", {"url": url, "width": video.base_width, "height": height}, True
        )
        item_id = _scene_item_id(client, scene_name, TICKER_SOURCE)
        client.set_scene_item_transform(scene_name, item_id, _ticker_transform(video, height))
    else:
        client.set_input_settings(TICKER_SOURCE, {"url": url}, True)
